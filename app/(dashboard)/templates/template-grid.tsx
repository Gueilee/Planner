...
// DropdownMenu (base-ui) em vez de um <div className="absolute"> desenhado à
// mão — este renderiza num portal (fora do container rolável da grade) e já
// vira sozinho pra cima quando não cabe embaixo (linha perto do fim da
// lista), em vez de ficar cortado pelo overflow sem jeito de rolar até ele.
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-64">
        <DropdownMenuItem onClick={() => onDuplicate(task.id)}>Duplicar linha</DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => onAddAbove(task.id)}>Adicionar nova linha acima</DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => onAddChild(task.id)}>Adicionar nova linha como última filha</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
}
